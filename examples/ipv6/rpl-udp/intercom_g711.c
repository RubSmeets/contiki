/*
 * Transmit audio with compression (G.711) and receive audio.
 */
...
tcpip_handler(void)
{
	...
	if(uip_newdata()) {
		...
		if(len == MAX_REC_PAYLOAD_LEN) {
			...
			PRINTFDEBUG("Capture on %d\n", appdata[5]);
			leds_on(LEDS_RED);

			/* Disable wait tone */
			play_tone = 0x00;

			/* Extract audio codec settings */
			audio_codec = appdata[5];
...
{
	uint8_t encodedSample = 0x00;
	int tempSample;

	tempSample = phidgets.value(PHIDGET3V_2);
	tempSample = tempSample - 2457;	/* remove offset (1,5V - ref 2,5V equals 60% of max value 4096) */

	switch(audio_codec) {	/* Calculated using little endian */
		case RFC3551_PCMA:
			encodedSample = linear2alaw((tempSample<<8) | ((tempSample>>8) & 0xFF)); /* ALAW encoded sample */
			break;
		case RFC3551_PCMU:
			encodedSample = linear2ulaw((tempSample<<8) | ((tempSample>>8) & 0xFF)); /* ULAW encoded sample */
			break;
		default:
			break;
	}

	audio_sample_buf[ptr] = encodedSample;
	ptr++;
